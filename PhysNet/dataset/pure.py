"""
PURE Dataset
Video data stored as images; Label data stored as JSON files
data/PURE/
|   |-- 01-01/
|      |-- 01-01/
|      |-- 01-01.json
...
|      |-- ii-jj/
|      |-- ii-jj.json
The videos have a frame rate of 30 Hz with a cropped resolution of 640x480 pixels and a 4.8mm lens.
Reference data delivers pulse rate wave and SpO2 readings with a sampling rate of 60 Hz.
Downsampled, cubic spline interpolation
"""
...
class Preprocess:
    ...
    def read_process(self):
        ...
        for i in progress_bar:
            ...
            with open(self.dirs[i] + os.sep + subject + ".json") as f:
                info = json.load(f)
            frames = self.read_video(self.dirs[i], subject, info)
    ...
    def preprocess(self, frames, gts):
        """
        Main part, resize -> normalize / standardize
