...
def resize(frames, dynamic_det, det_length,
    ...
    """
    :param frames:
    :param dynamic_det: whether to perform dynamic detection
...
def facial_detection(detector, frame, larger_box=False, larger_box_size=1.0):
    """
    Uses MTCNN to detect face region
    :param detector:
    :param frame:
    :param larger_box: whether to enlarge bbox, for handling movement
    :param larger_box_size:
    """
    ...
    if len(face_zone) >= 2:
        print("Warning: More than one face detected (Only cropping the biggest one).")
...
def get_blocks(frame, h_num=5, w_num=5):
    ...
    w_len = w // w_num
    ret = []
    h_idx = [i * h_len for i in range(0, h_num)]
    w_idx = [i * w_len for i in range(0, w_num)]
    for i in h_idx:
        for j in w_idx:
            ret.append(frame[i: i + h_len, j: j + w_len, :])
    return ret
...
def get_STMap(frames, chunk_length=300, roi_num=25, chunk_stride=-1) -> np.ndarray:
    ...
    :param hrs:
    :param chunk_length:
    :param roi_num: number of divided blocks, 5 * 5 = 25
    ...
    for i in range(0, len(frames) - chunk_length + 1, chunk_stride):
        temp = np.zeros((chunk_length, roi_num, 3))
...
class MyDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        y = torch.from_numpy(gt[:, 0])
        hr = torch.from_numpy(gt[:, 1])
